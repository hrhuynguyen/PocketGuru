...
export async function buildPdfFromImages(files) {
    if (files.length === 0)
...
async function imageBytesForPdf(file) {
...
async function rasterizeImage(file) {
    ...
    if (!ctx)
        throw new Error('Could not prepare this image for PDF export.');
    ...
    return new Promise((resolve, reject) => {
        canvas.toBlob((blob) => {
            if (blob)
                ...
                reject(new Error('Could not convert this image for PDF export.'));
        }, 'image/jpeg', 0.9);
    });
}
